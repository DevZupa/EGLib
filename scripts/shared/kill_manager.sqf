	...
	if ( _unit isKindOf "Man" ) then {
		if ( side (group _unit) == GRLIB_side_civilian ) then {
			...
			if ( isPlayer _killer ) then {
				...
				if ( GRLIB_civ_penalties ) then {
					...
					[[[  format["%1 (CIV)",name _unit], -1 ]], _killer] call zupa_killMessage;
				};
		...
		if ( side _killer == GRLIB_side_friendly ) then {
			if ( side (group _unit) == GRLIB_side_enemy ) then {
				...
				if ( isplayer _killer ) then {
                    ...
                    [[[  format["%1",name _unit], 1 ]], _killer] call zupa_killMessage;
                    diag_log format['ZUPAA'];
					// enemy kill
			...
			if ( side (group _unit) == GRLIB_side_friendly ) then {
                ...
                [[[  format["%1 (Friendly)",name _unit], -1 ]], _killer] call zupa_killMessage;
				// team kill
			};
		} else {
			if ( side (group _unit) == GRLIB_side_friendly ) then {
		...
		if ( typeof _unit in all_hostile_classnames ) then {
			...
			if ( isplayer _killer ) then {
				...
				[[[  format["%1",typeOf _unit], 1 ]], _killer] call zupa_killMessage;
